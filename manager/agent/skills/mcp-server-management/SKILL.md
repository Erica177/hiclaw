...
- User asks you to enable a new integration or rotate a credential
...
Only `mcp-github.yaml` is built-in. For any other service, generate the YAML config based on the user's API description, then deploy it with the setup script.
...
- Credentials are stored only in Higress MCP Server config — Workers never see them directly
...
## Calling MCP Tools Directly (mcporter)

You can call any configured MCP server's tools directly via `mcporter`. Your config is at `~/mcporter-servers.json` (auto-generated by the setup script).
...
Use `list <server-name> --schema` first if you're unsure what parameters a tool requires — it shows each tool's description, argument types, and which are required vs optional.

When to use: when you need to interact with external APIs (e.g., search GitHub repos, create issues, call a weather API) without delegating to a Worker. This is useful for quick lookups or when you need the result to make a decision.
...
The setup script handles authorization automatically. For manual adjustments:
